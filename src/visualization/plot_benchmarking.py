    ...
    if PLOT:
        ...
        for mode in modes:
            if mode == "constrained":
                ...
                loss_data_array = np.array(
                    [[loss["loss_data_after_projection"] for loss in losses] for losses in losses_runs]
                )  # Shape (N, epochs)
                loss_displacement_array = np.array(
                    [[loss["loss_displacement"] for loss in losses] for losses in losses_runs]
                )  # Shape (N, epochs)
                loss_before_projection_array = np.array(
                    [[loss["loss_data_before_projection"] for loss in losses] for losses in losses_runs]
                )  # Shape (N, epochs)
                ...
                mean_loss_before_projection = np.mean(loss_before_projection_array, axis=0)
                std_loss_before_projection = np.std(loss_before_projection_array, axis=0)

                # Plot after projection
                loss_data_plot.plot(
                    epochs_range, mean_loss_data, color="blue", linestyle="-", label="Constrained After Projection"
                )
                ...
                losses_runs = losses_runs_dict[mode]
                loss_unconstrained_array = np.array(
                    [[loss["loss_unconstrained"] for loss in losses] for losses in losses_runs]
                )  # Shape (N, epochs)
                ...
                loss_data_plot.plot(
                    epochs_range, mean_loss_unconstrained, color="green", linestyle="-", label="Unconstrained"
                )
                loss_data_plot.fill_between(
                    epochs_range,
        ...
        if len(functions) == 2:
            try:
                for i, func in enumerate(functions):
                    ...
                    ax.scatter(
                        test_inputs_list_dict[modes[0]][0],
                        test_outputs_list_dict[modes[0]][0][:, i],
                        c="red",
                        label="Test",
                    )
        ...
        for mode in modes:
            test_predictions_array = np.array(
                test_predictions_list_dict[mode]
            )  # Shape: [N, num_samples, num_functions]
                    ...
                    for i, func in enumerate(functions):
                        if mode == "constrained":
                            ...
                            ax_pred.errorbar(
                                test_inputs,
                                mean_predictions[:, i],
                                yerr=std_predictions[:, i],
                                fmt="o",
                                ecolor="gray",
                                capsize=3,
                                label=f"{mode.capitalize()} Prediction",
                            )
                            ...
                            ax_proj.errorbar(
                                test_inputs,
                                mean_predictions_before[:, i],
                                yerr=std_predictions_before[:, i],
                                fmt="o",
                                ecolor="orange",
                                capsize=3,
                                label=f"{mode.capitalize()} Prediction Before Projection",
                            )
                            ax_proj.errorbar(
                                test_inputs,
                                mean_predictions[:, i],
                                ...
                                ecolor="gray",
                                capsize=3,
                                label=f"{mode.capitalize()} Prediction After Projection",
                            )
...
def plot_sin_ineq_results(
    ...
    def _stats(arr_list):
        ...
        return (np.mean(arr[:, idx, 0], axis=0), np.std(arr[:, idx, 0], axis=0))
    ...
    if "constrained" in test_prediction_before_projection_list_dict:
        bp_mean, bp_std = _stats(test_prediction_before_projection_list_dict["constrained"])

    # ── Constraint residual helpers ──────────────────────────────────────────
    def g1(x, y):
        return y - (1.0 + x**2 / (3.0 * np.pi**2))  # upper:  ≤ 0

    def g2(x, y):
        return -(1.0 + x**2 / (3.0 * np.pi**2)) - y  # lower:  ≤ 0
    ...
    def _draw_corridor(ax, label=True):
        ax.fill_between(x_d, lower_d, upper_d, color="gray", alpha=0.12, label="Feasible corridor" if label else None)
        ax.plot(x_d, upper_d, "k--", lw=1.2, label="$\\pm(1+x^2/(3\\pi^2))$ envelope" if label else None)
        ax.plot(x_d, lower_d, "k--", lw=1.2)

    # ── [0,0]  Corridor + ground truth + predictions ──────────────────────────
    ax = axes[0, 0]
    _draw_corridor(ax)
    ax.plot(x_d, y_true_d, "k-", lw=2.0, label="True $y = (1+x^2/(3\\pi^2))\\sin(x)$")
    for m in modes:
        mn, sd = pred_stats[m]
    ...
    ax.scatter(x_tr[~viol_tr], y_tr[~viol_tr], s=8, alpha=0.30, color="tab:blue", label="Feasible labels")
    ax.scatter(
        x_tr[viol_tr],
        y_tr[viol_tr],
        s=14,
    ...
    ax.set_xlabel("$x$")
    ax.set_ylabel("$y$")
    ax.set_title(f"Training data: {viol_tr.sum()}/{len(y_tr)} labels violate corridor")
    ax.legend()
    ...
    for m in modes:
        ...
        ax.plot(x_s, g1v, lw=1.6, color=colors[m], label=f"{labels_[m]} — violations: {nviol}/{len(x_s)}")
    g1_max = max(max(g1(x_s, pred_stats[m][0]).max() for m in modes), 0)
    if g1_max > 1e-4:
        ax.fill_between(x_s, 0, g1_max * 1.05, alpha=0.07, color="red", label="Infeasible ($g_1 > 0$)")
    ax.set_xlabel("$x$")
    ...
    for m in modes:
        ...
        ax.plot(x_s, g2v, lw=1.6, color=colors[m], label=f"{labels_[m]} — violations: {nviol}/{len(x_s)}")
    g2_max = max(max(g2(x_s, pred_stats[m][0]).max() for m in modes), 0)
    if g2_max > 1e-4:
        ax.fill_between(x_s, 0, g2_max * 1.05, alpha=0.07, color="red", label="Infeasible ($g_2 > 0$)")
    ax.set_xlabel("$x$")
